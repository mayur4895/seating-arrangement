...
export async function addStudent(formData){ 
    try{
        ...
        const res = await fetch(`http://localhost:3000/api/auth/student/singlestudent`,Options)
...
export async function  SearchStudent(query){  
    try{
        ...
        const res = await fetch(`http://localhost:3000/api/auth/student/search/?query=${query}`) 
        const json = await res.json(); 
        return json; 
